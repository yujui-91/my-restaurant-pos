...
df_batches = pd.read_sql_query('''
    ...
    WHERE s.prod_id LIKE ? AND p.status = 1
...
if not df_batches.empty:
    options = df_batches.apply(
        lambda r: f"【批次編號: {r['batch_id']}】 品項: {r['prod_id']}-{r['prod_name']} (現存:{r['qty']}{r['use_unit']} | 效期:{r['expiry_date']})", 
        axis=1
    ).tolist()
    
    selected_row = st.selectbox("🔍 選擇要微調/報廢的特定項目與批次：", options)
    
    batch_id_part = int(selected_row.split("【批次編號: ")[1].split("】")[0])
    ...
    with st.form("inventory_adjustment_form"):
        ...
        if submit_adj:
            ...
            if new_total_qty < 0:
                ...
                log_details = f"庫存微調【{item_name}】(批次編號 {batch_id_part})。動作：{adj_type}，數量：{adj_qty} {unit_label}，異動後現存：{new_total_qty} {unit_label}。原因：{final_reason}"
