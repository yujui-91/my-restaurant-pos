...
# 💡 修正 1：移除 price >= 0 的舊限制，改拉出 status 欄位來判斷是否下架
...
if not df_stock.empty:
    # 轉換狀態碼為可讀文字
    df_stock['狀態'] = df_stock['狀態碼'].apply(lambda s: "🔴 已下架停用" if s == 0 else "🟢 正常使用中")
    
    # 重新調整欄位順序，把狀態放到前面，並隱藏狀態碼
    show_cols = ['批次編號', '編號', '商品名稱', '狀態', '庫存量', '單位', '有效期限', '安全庫存', '供應商', '供應商電話']
    df_display = df_stock[show_cols]
    
    # 💡 修正 2：高亮紅色邏輯（若狀態為下架，該列的狀態欄位變紅底白字）
    def highlight_disabled(row):
        return ['background-color: #ffcccc; color: #cc0000; font-weight: bold;' if row['狀態'] == "🔴 已下架停用" else '' for _ in row]
    
    # 使用 style.apply 依照整列資料進行條件上色
    st.dataframe(df_display.style.apply(highlight_disabled, axis=1), use_container_width=True)
    
    # ==========================================
    # 💡 修正 3：不想要看到時，提供徹底刪除下架品項庫存的功能
    # ==========================================
    ...
    if not disabled_items.empty:
        ...
        if st.button("❌ 確認從庫存明細中刪除此批次", type="primary"):
            ...
            # 寫入歷史紀錄
            from database.db_core import log_history, trigger_toast
